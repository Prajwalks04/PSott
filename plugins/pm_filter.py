...
async def next_page(bot, query):
                    ...
                    btn = [
                        [
                            InlineKeyboardButton(
                                ...
                                url=await get_shortlink(f"https://telegram.me/PS_OTT_BOT?start=files_{file.file_id}")
                            ),
                        ]
                        for file in files
                    ]
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/BY_PASS_LINK_04')
        ] 
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('♥️ Love @PS_OTT ♥️')
            else:
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            await query.answer(url=f"https://t.me/PS_OTT_BOT?start={ident}_{file_id}")
        except Exception as e:
            await query.answer(url=f"https://t.me/PS_OTT_BOT?start={ident}_{file_id}")
        ...
        buttons = [[
            InlineKeyboardButton('➕↖️ Add Me To Your Groups ↗️➕', url='http://t.me/PS_OTT_BOT?startgroup=true')
        ], [
            InlineKeyboardButton('🧞‍♀️ Search 🧐', switch_inline_query_current_chat=''),
            InlineKeyboardButton('🔔 Updates 🤖', url='https://t.me/PS_OTT')
        ], [
            InlineKeyboardButton('🙆🏻 Help 🦾', callback_data='help'),
            InlineKeyboardButton('♥️ About ♥️', callback_data='about')
        ],[
            InlineKeyboardButton('Watch Tutorial', url='https://t.me/BY_PASS_LINK_04')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await query.answer('♥️ Thank You PS Developer ♥️')
        ...
        try:
            btn = [[
                ...
                InlineKeyboardButton(text=f"🐞 REPORT BUG 🐞", url=f"https://telegram.me/PSMOVIESBOT")
            ],[
                InlineKeyboardButton(text=f"⚡️ Learn Bot Making 🦋", url=f"https://t.me/PS_OTT")
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        ...
        if URL_MODE is True:
            if message.from_user.id in ADMINS:
                ...
                btn = [
                    [
                        InlineKeyboardButton(text=f"{file.file_name}", url=await get_shortlink(f"https://telegram.me/PS_OTT_BOT?start=files_{file.file_id}")),
                        InlineKeyboardButton(text=f"[{get_size(file.file_size)}]", url=await get_shortlink(f"https://telegram.me/PS_OTT_BOT?start=files_{file.file_id}")),
                    ]
                    for file in files
                ]
        else:
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ to ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/BY_PASS_LINK_04'),
        ] 
    )
    if offset != "":
